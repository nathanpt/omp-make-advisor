...
test("block missing Evidence parses with empty evidence, siblings kept", () => {
	const text = [
		"# Advisor brief",
		"",
		...
		"Rationale: evidence line absent — a custom consideration.",
		"",
	...
	assert.deepEqual(
		parsed.candidates.map((c) => [c.id, c.evidence]),
		[
			["conc-1", ""],
			["err-1", "src/auth/session.ts:88"],
		],
	);
});

test("evidence-less candidate round-trips with the Evidence: line omitted", () => {
	const custom: readonly BriefCandidate[] = [
		{ id: "custom-1", title: "Keep functions under 30 lines", evidence: "", status: "keep", rationale: "simplicity standard" },
	];
	const text = briefToText(custom);
	assert.ok(!text.includes("Evidence:"), "evidence-less block must not render an Evidence line");
	assert.deepEqual(parseBrief(text), { candidates: custom, skipped: 0 });
});

test("verifyEvidenceAnchors never fails an evidence-less candidate", () => {
	assert.deepEqual(verifyEvidenceAnchors("/nonexistent-cwd", [{ id: "custom-1", title: "t", evidence: "" }]), []);
});
...
test("readEvidenceContext: ranges, caps, bare files, and non-file evidence", () => {
	...
	try {
		...
		// Directory (area guard), missing path, past-EOF start, and empty
		// (custom consideration): no context.
