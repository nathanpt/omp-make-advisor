...
// The scan turn runs outside the command handler (pi.sendUserMessage starts an
// agent turn), so progress is event-driven: tool_execution_end counts scout
// batches returning, agent_end closes the phase and reads back the brief.
const scanState = { active: false, batches: 0 };

function runScan(pi: ExtensionAPI, ctx: ExtensionCommandContext): void {
...
async function runPicker(ctx: ExtensionCommandContext): Promise<void> {
	...
	if (!read.ok) {
		ctx.ui.notify(`oma: no advisor-brief.md in ${ctx.cwd} — run /oma scan first`, "warning");
		return;
	}
	...
	const traps: CandidateTrap[] = candidates.map(({ id, title, evidence }) => ({ id, title, evidence }));
	const initialStates = new Map(candidates.map((c) => [c.id, c.status]));
	const result = await ctx.ui.custom<PickerResult | undefined>(
		(_tui, _theme, keybindings, done) => new TrapPicker(traps, keybindings, done, initialStates),
		{ overlay: true },
	);
	if (!result) {
...
export default function omaExtension(pi: ExtensionAPI): void {
	...
	const commandOptions = {
		...
		handler: async (args: string, ctx: ExtensionCommandContext): Promise<void> => {
			if (args.trim() === "scan") return runScan(pi, ctx);
